...
import { AuthResponse, RegisterManagerData, Shop, Mall, UserResponse, Product, DashboardSummary, WeeklySale, TopProduct, RecentOrder, DashboardAlerts, ReportDateParams, SalesSummary, SalesTrendItem, SalesTopProduct, SalesBestSeller } from './authTypes';
...
export const authAPI = {
  ...
  getDashboardSummary: async (shopId: number): Promise<DashboardSummary> => {
    try {
      const response = await api.get(`/api/dashboard/summary/${shopId}`);
      console.log('[authAPI.getDashboardSummary] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getDashboardSummary] Error:', error);
      if (error instanceof AxiosError && error.response) {
  ...
  getWeeklySales: async (shopId: number): Promise<WeeklySale[]> => {
    try {
      const response = await api.get(`/api/dashboard/weekly-sales/${shopId}`);
      console.log('[authAPI.getWeeklySales] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getWeeklySales] Error:', error);
      if (error instanceof AxiosError && error.response) {
        throw new Error(error.response.data.message || 'Failed to fetch weekly sales');
      }
      throw new Error('An unexpected error occurred');
    }
  },

  getDashboardTopProducts: async (shopId: number): Promise<TopProduct[]> => {
    try {
      const response = await api.get(`/api/dashboard/top-products/${shopId}`);
      console.log('[authAPI.getDashboardTopProducts] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getDashboardTopProducts] Error:', error);
      if (error instanceof AxiosError && error.response) {
  ...
  getRecentOrders: async (shopId: number): Promise<RecentOrder[]> => {
    try {
      const response = await api.get(`/api/dashboard/recent-orders/${shopId}`);
      console.log('[authAPI.getRecentOrders] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getRecentOrders] Error:', error);
      if (error instanceof AxiosError && error.response) {
  ...
  getDashboardAlerts: async (shopId: number): Promise<DashboardAlerts> => {
    try {
      const response = await api.get(`/api/dashboard/alerts/${shopId}`);
      console.log('[authAPI.getDashboardAlerts] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getDashboardAlerts] Error:', error);
      if (error instanceof AxiosError && error.response) {
  ...
  getSalesSummary: async (shopId: number, params?: ReportDateParams): Promise<SalesSummary> => {
    try {
      const response = await api.get(`/api/reports/sales/summary/${shopId}`, { params });
      console.log('[authAPI.getSalesSummary] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getSalesSummary] Error:', error);
      if (error instanceof AxiosError && error.response) {
  ...
  getSalesTrend: async (shopId: number, params?: ReportDateParams): Promise<SalesTrendItem[]> => {
    try {
      const response = await api.get(`/api/reports/sales/trend/${shopId}`, { params });
      console.log('[authAPI.getSalesTrend] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getSalesTrend] Error:', error);
      if (error instanceof AxiosError && error.response) {
  ...
  getSalesTopProducts: async (shopId: number, params?: ReportDateParams): Promise<SalesTopProduct[]> => {
    try {
      const response = await api.get(`/api/reports/sales/top-products/${shopId}`, { params });
      console.log('[authAPI.getSalesTopProducts] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getSalesTopProducts] Error:', error);
      if (error instanceof AxiosError && error.response) {
        throw new Error(error.response.data.message || 'Failed to fetch sales top products');
      }
      throw new Error('An unexpected error occurred');
    }
  },

  getSalesBestSellers: async (shopId: number, params?: ReportDateParams): Promise<SalesBestSeller[]> => {
    try {
      const response = await api.get(`/api/reports/sales/best-sellers/${shopId}`, { params });
      console.log('[authAPI.getSalesBestSellers] Response:', JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error: unknown) {
      console.error('[authAPI.getSalesBestSellers] Error:', error);
      if (error instanceof AxiosError && error.response) {
